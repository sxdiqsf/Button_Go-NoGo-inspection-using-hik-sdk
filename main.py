...
from PyUICBasicDemo1 import Ui_MainWindow,apply_styles
import ctypes
import glob
...
def save_debug_result(original_img, crop_img, edges_img, masked_img, result, score,
    ...
    DEBUG_SAVE_LIMIT = 5000
    os.makedirs(save_dir, exist_ok=True)

    TARGET_H = 300
    # ── Enforce rolling limit ──
    existing = sorted(glob.glob(os.path.join(save_dir, "*.jpg")))  # oldest first
    while len(existing) >= DEBUG_SAVE_LIMIT:
        os.remove(existing.pop(0))  # delete oldest
        print(f"[DEBUG SAVE] Limit hit ({DEBUG_SAVE_LIMIT}), deleted oldest.")
